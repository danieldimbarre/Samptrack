...
const { getServerOnline, getServerWebSite, callWithTimeout} = require('./samp')
    ...
    case 'SAMP':
      ...
      if (!images.has(serverRegistration.data.ip)) {
        getServerWebSite(serverRegistration.data.ip, serverRegistration.data.port || 7777, async (err, res) => {
          ...
          try {
            let image = new Buffer(await (await fetch('https://www.google.com/s2/favicons?domain=' + res + '&sz=128')).arrayBuffer());

            if (image.length !== 726) {
              let base64String = btoa(String.fromCharCode.apply(null, new Uint8Array(image)));
